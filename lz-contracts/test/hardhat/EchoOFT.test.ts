...
describe('EchoOFT Test', function () {
    // Constant representing a mock Endpoint ID for testing purposes
    const eidA = 1
    const eidB = 2
    const initialAmount = ethers.utils.parseEther('100')
    // Declaration of variables to be used in the test suite
    let EchoOFT: ContractFactory
    let EndpointV2Mock: ContractFactory
    let ownerA: SignerWithAddress
    let ownerB: SignerWithAddress
    let endpointOwner: SignerWithAddress
    let EchoOFTA: Contract
    let EchoOFTB: Contract
    ...
    before(async function () {
        ...
        EchoOFT = await ethers.getContractFactory('EchoOFT')
    ...
    beforeEach(async function () {
        ...
        // Deploying two instances of EchoOFT contract with different identifiers and linking them to the mock LZEndpoint
        EchoOFTA = await EchoOFT.deploy(
            'aOFT',
            'aOFT',
            initialAmount,
        ...
        // Setting each EchoOFT instance as a peer of the other in the mock LZEndpoint
